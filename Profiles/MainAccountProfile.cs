    ...
    public class MainAccountProfile : Profile
    {
        public MainAccountProfile()
        {
            CreateMap<string[], MainAccount>()
                ...
                .ForMember(dest => dest.Status, opt => opt.MapFrom(src => src[12]))
                .ForMember(dest => dest.ClearBalance, opt => opt.MapFrom(src => src[13]))
                .ForMember(dest => dest.AccountGroup, opt => opt.MapFrom(src => src[14]))
                .ForMember(dest => dest.AccountCategory, opt => opt.MapFrom(src => src[15]))
                .ForMember(dest => dest.AccountType, opt => opt.MapFrom(src => src[16]))
                .ForMember(dest => dest.CashFlowsType, opt => opt.MapFrom(src => src[17]))
